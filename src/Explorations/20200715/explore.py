...
chemicalB_dict = {}
viability_mean_list = []
with open('seedEffect.csv', 'r') as file_read :
	data =  file_read.readlines()
	for line in data[7:] :
		line = line.replace('\"', '').split(',')

		run_number,step, viability, remainingCellRatio, seed = int(line[0]), float(line[1]), float(line[2]), float(line[3]), float(line[4])
		print(run_number,seed)
		if (run_number,seed)  not in viability_dict :
			viability_dict[(run_number,seed)] = []
		viability_dict[(run_number,seed)].append(viability)
		if (run_number,seed) not in remaining_dict :
			remaining_dict[(run_number,seed)] = []
		remaining_dict[(run_number,seed)].append(remainingCellRatio)	
