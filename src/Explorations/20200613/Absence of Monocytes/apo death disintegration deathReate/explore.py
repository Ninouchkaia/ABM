...
with open('table1-point.csv', 'r') as file_read :
	...
	for line in data[7:] :
		...
		run_number,apoThreshold,apoDuration,disintegrationDuration,deathRate,viability,remainingCellRatio = int(line[0]), float(line[1]), float(line[2]), float(line[3]), float(line[4]), float(line[5]),float(line[6]),float(line[7])
