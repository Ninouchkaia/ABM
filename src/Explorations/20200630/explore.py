...
chemicalB_dict = {}
viability_mean_list = []
with open('.MacrophageDifferentiationModel10.csv', 'r') as file_read :
	data =  file_read.readlines()
	for line in data[7:] :
		line = line.replace('\"', '').split(',')
		"[run number]","gui-LateApoThreshold","gui-apoptosisDuration","gui-disintegrationDuration","gui-death_rate","alpha","[step]","getViability","getRemainingCellRatio"

		run_number, deathRate, alpha, step, viability, remainingCellRatio = int(line[0]), float(line[4]), float(line[5]), int(line[6]), float(line[7]),float(line[8])
		print(run_number,deathRate, alpha, step, viability, remainingCellRatio)
		if (run_number,deathRate, alpha)  not in viability_dict :
			viability_dict[(run_number,deathRate, alpha)] = []
		viability_dict[(run_number,deathRate, alpha)].append(viability)
		if (run_number,deathRate, alpha) not in remaining_dict :
			remaining_dict[(run_number,deathRate, alpha)] = []
		remaining_dict[(run_number,deathRate, alpha)].append(remainingCellRatio)	
