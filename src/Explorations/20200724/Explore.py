...
viability_dict = {}
remaining_dict = {}
with open('ABM_Probabilistic_Explo_Death_BlockApo_DisintegrationDuration.csv', 'r') as file_read :
	data =  file_read.readlines()
	for line in data[7:] :
		line = line.replace('\"', '').split(',')
		"[run number]", "gui-DeathThreshold","gui-BlockedApoptosisThreshold", "gui-disintegrationDuration","[step]","getViability","getRemainingCellRatio"

		run_number, death, blockapo, disintegration, step, viability, remainingCellRatio = int(line[0]), int(line[1]), int(line[2]), int(line[3]), int(line[3]), float(line[5]),float(line[6])
		if (run_number,death, blockapo, disintegration)  not in viability_dict :
			viability_dict[(run_number,death, blockapo, disintegration)] = []
		viability_dict[(run_number,death, blockapo, disintegration)].append(viability)
		if (run_number,death, blockapo, disintegration) not in remaining_dict :
			remaining_dict[(run_number,death, blockapo, disintegration)] = []
		remaining_dict[(run_number,death, blockapo, disintegration)].append(remainingCellRatio)	

# print(len(viability_dict), len(remaining_dict))
# for i in viability_dict :
# 	print(i, viability_dict[i])


df_viability = pd.DataFrame.from_dict(viability_dict, orient='index')
df_viability = df_viability.transpose()
df_remaining = pd.DataFrame.from_dict(remaining_dict, orient='index')
df_remaining = df_remaining.transpose()
print(df_viability)
